...
func InsertLog(user *usersModels.User, resource string, action string) {
	...
	_, err := core.DB.Exec(query, user.ID, user.Name, user.Email, resource, action)
	if err != nil {
		panic(err)
	}
...
func GetLogsFromDB(search, sort, order, page, pageSize string) []usersModels.Log {
	...
	query := "SELECT id, user_id, name, email,  resource, action, timestamp FROM logs WHERE 1=1"
	args := []interface{}{}
	argIndex := 1
	...
	for rows.Next() {
		...
		err := rows.Scan(&l.ID, &l.UserID, &l.Name, &l.Email, &l.Resource, &l.Action, &l.Timestamp)
		if err != nil {
			panic(err)
		}
		logs = append(logs, l)
