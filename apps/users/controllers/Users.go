...
func GetUsersFromDB(search, sort, order, page, pageSize string) []usersModels.User {
	...
	query := "SELECT id, name,local_name,badge_id, email, active, online, last_login, department,job_title FROM users WHERE 1=1"
	args := []interface{}{}
	argIndex := 1
	...
	for rows.Next() {
		...
		err := rows.Scan(&u.ID, &u.Name,&u.LocalName,&u.BadgeId, &u.Email, &u.Active, &u.Online, &u.LastLogin, &u.Department, &u.JobTitle)
		if err != nil {
			panic(err)
		}
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	query := "select id, email, name, active from users where id = $1"

	var user1 usersModels.User

	err := core.DB.QueryRow(query, userID).Scan(&user1.ID, &user1.Email, &user1.Name, &user1.Active)
	if err != nil {
	...
	fmt.Print(CurrentUser.ID, " : ", CurrentUser.Name, "  sent the following ", Username, " : ", Email, " : ", Active, " to update the User ID : ", userID)
