...
func InsertLog(user *usersModels.User, resource string, action string) {
	...
	fmt.Printf("\n Insert Log %d  by %s  for %s with action: %s\n", user.ID, user.Name, resource, action)

	// TODO: implement log insertion to database, create a new log record with user id, resource, action and timestamp
	query := "INSERT INTO logs (user_id, name, email, resource, action) VALUES ($1, $2, $3, $4, $5)"
	_, err := DB.Exec(query, user.ID, user.Name, user.Email, resource, action)
	if err != nil {
		panic(err)
	}
...
func GetUserByEmail(email string) *usersModels.User {
	...
	email = strings.ToLower(strings.TrimSpace(email))
	 ...
	 query := `
        SELECT 
            u.id, u.name, u.local_name,u.badge_id, u.email, u.active, u.online, u.last_login, u.created_at, u.updated_at, 
            r.id, r.name, r.description,
            p.id, p.resource, p.action
        FROM users u
        LEFT JOIN users_roles ur ON u.id = ur.user_id
    ...
    for rows.Next() {
        ...
        err := rows.Scan(
            &user.ID, &user.Name,&user.LocalName,&user.BadgeId, &user.Email, &user.Active, &user.Online, &user.LastLogin, &user.CreatedAt, &user.UpdatedAt, 
            &rID, &rName, &rDesc,
            &pID, &pResource, &pAction,
        )
        if err != nil {
